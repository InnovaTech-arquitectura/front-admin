...
export class CapacitacionesService {
	...
	private handleError(error: any): Observable<never> {
		...
		if (error.status === 400) {
			errorMessage =
				'Solicitud incorrecta. Verifica los datos ingresados. Recuerda:(1) Todos los campos son obligatorios (2) Los cupos deben ser mayores a 1 (3) La fecha debe ser mayor a la actual';
	...
	findAll(): Observable<any> {
		...
		return this.http.get<any>(this.apiURL + '/all', { headers }).pipe(catchError(this.handleError));
	}
	...
	updateCourse(course: Course): Observable<any> {
		...
		const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
		return this.http
			.put(this.apiURL + '/' + course.id, course, { headers, responseType: 'text' })
	...
	addCourse(course: Course): Observable<any> {
		...
		const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
		return this.http
			.post(this.apiURL + '/new', course, { headers, responseType: 'text' })
			.pipe(catchError(this.handleError));
	}
